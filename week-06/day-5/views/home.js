...
function changeImageAndTitles(picNumber) {
  ...
  document.getElementsByClassName("galleryname")[0].innerHTML =
    gallerypics1[i].name;
  document.getElementsByClassName("gallerytitle")[0].innerHTML =
    gallerypics1[i].title;
}
...
function randomPicGenerator() {
  let newNumber = 0;
  while (newNumber === i) {
    newNumber = Math.floor(Math.random() * 4) + 0;
  }
    changeImageAndTitles(newNumber);
}
